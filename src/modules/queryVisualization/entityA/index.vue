...
<template>
    <div class="queryvisualization-entitya">
    <div style="color:#9e9c9c;">你想要从一段关系的哪个实体开始查询？</div>
        <!--ion-transgender-->
    <div class="marg-top"><span class="ion-social-codepen-outline" style="color: #0e9ee2;font-size: large;"></span><span style="padding-left: 10px;">实体 A</span> </div>
    <div class="marg-conent">
        <div>
            <div class="select-lable">选择一个实体模型</div>
            <ad-select :small="true" @select="entityselect" :disabled="$parent.isSnapshoot" placeholder="请选择" :options="$parent.queryentity.vertexInputA.add_vertexOption" v-model="$parent.queryentity.vertexInputA.add_vertexOption_select"></ad-select>
        </div>

        <div style="margin-top: 10px;">
            <div class="mult-lable">属性筛选（可选）</div>
            <div style="padding-left: 126px;width:200px;" v-if="$parent.queryentity.vertexInputA.ruleType == 'and'" @click="checkmodule">
        ...
        <div v-for="entity in $parent.queryentity.vertexInputA.properties" class="condition" >
            <div class="cond-line"></div>
            <span class="cond-lab">{{$parent.queryentity.vertexInputA.ruleType}}</span>
            <ad-select :small="true"  @select="propertyselect($event,entity)" :disabled="$parent.isSnapshoot" style="width:160px;" class="cond-marg" :options="$parent.queryentity.vertexInputA.add_nameOption" v-model="entity.add_nameOption_select"></ad-select>
    ...
    export default {
        ...
        methods: {
            initdata(){
                ...
                if(projectInfo) {
                    let temparray = [];
                    queryentityselect({projectCode: projectInfo.projectCode}, (res) => {
                        res.data.forEach((m) => {
                        temparray.push({text: m.entityName, value: m});
                })
                    if (temparray.length > 0) {
                        _this.$parent.queryentity.vertexInputA.add_vertexOption = temparray;
                        _this.$parent.queryentity.vertexInputA.add_vertexOption_select = temparray[0].value;
                        _this.$parent.queryentity.vertexInputA.vertexCode = temparray[0].value.dataId;
                        _this.initproperty();
                        _this.$parent.$refs.refrelation.initrelation();
                    }
                });
                }
            },
            initproperty(){
                let _this =this;
                    ...
                    propertyselectlist({
                        "propertyName":"",
                        "busType":0,
                        "dataId":this.$parent.queryentity.vertexInputA.vertexCode
                    },(res)=>{
                        res.data&&res.data.forEach((m)=>{
                        temproperlist.push({text:m.propertyName,value:m});
                    });
                    _this.$parent.queryentity.vertexInputA.add_nameOption =temproperlist;
                });
            },
            checkmodule(){
                ...
                {
                this.$parent.queryentity.vertexInputA.ruleType=this.$parent.queryentity.vertexInputA.ruleType=='and'?'or':'and';
                }
            },
            addcondition(){
                if(this.$parent.queryentity.vertexInputA.vertexCode&&!this.$parent.isSnapshoot){
                    this.$parent.queryentity.vertexInputA.properties.push({
                        "name": "",
                        "values": [],
                        "rule": "",
            ...
            removeentity(entity){
                if(!this.$parent.isSnapshoot) {
                    let tempprop = this.$parent.queryentity.vertexInputA.properties;
                    this.$parent.queryentity.vertexInputA.properties = tempprop.filter((m) => {return entity != m}
                );
                }
            },
            entityselect(entity){
                if(this.$parent.queryentity.vertexInputA.vertexCode) {
                    if (this.$parent.queryentity.vertexInputA.vertexCode != entity.value.dataId)
                    {
                        this.$parent.queryentity.vertexInputA.vertexCode = entity.value.dataId;
                        this.$parent.queryentity.vertexInputA.properties = [];
                        this.initproperty();

                    this.$parent.queryentity.edgeInput = {
                        "edgeCode": "",
                        ...
                        this.$parent.$refs.refrelation.initrelation();

                    }
                }
                else{
                    this.iscallpack = true;
                }
            },
