...
<template>
  <div class="admin-datepicker" :class="datesPopupShow ? 'active ' + classes : classes">
    <div class="admin-datepicker-label-text" v-if="label" @click.stop="labelClick">{{ label }}</div>
    <div class="admin-datepicker-container">
      <admin-input
        ...
        icon-class="ion-android-calendar"
        @focus="focus"
        :small="small"
        :disabled="disabled"
        @click.native.stop="() => {}">
...
<script>
  ...
  export default {
    ...
    mounted () {
      let vm = this
      window.document.addEventListener('click', function adDatepickerHide () {
        vm.datesPopupShow = false
      })
    },
    destroyed () {
      window.document.removeEventListener('click', 'adDatepickerHide')
    },
    ...
    watch: {
      ...
      inputValue (v) {
        ...
        if (this.datesPopupShow) this.render(res)
      },
      datesPopupShow (v) {
        if (v) this.$emit('focus', this.inputValue)
        else this.$emit('blur', this.inputValue)
      }
    },
    methods: {
      monthDayCount (year) {
      ...
      render (value) {
        let isEmptyString = this.isEmptyString
      ...
      isSelected (dateObj) {
        ...
        if (this.isEmptyString(this.inputValue)) return false
      ...
      tdClass (dateObj) {
        ...
        if (this.isEmptyString(this.inputValue)) {
