...
(function () {
	...
	p.listeners={};
	//p.parent=null;
	p.getGUID=function(){ return Painter.randStr(32); };
    p.children = [];
	p.hitChildren= [];
	p.oldHitChildren=[];
    /**
...
(function () {
    ...
    function Stage(arg) {
		...
		/*
		this.getCanvasPosition=function(canvas){
		  return { 
			x: canvas.offsetLeft, 
			y: canvas.offsetTop 
		  };
		...
		this.getMouse=function(e){
		  var canvasPosition=this.getCanvasPosition(this.canvas);
		  return { 
				x: e.pageX - canvasPosition.x, 
				y: e.pageY - canvasPosition.y 
			} ;
		...
		this.hitChildAll=function(children,hitCtx,mouse,cb){
		    for(var i=children.length-1;i>-1;i--){
			    var kid=children[i];
	            kid.draw(hitCtx);
				var f=Painter.testHit(mouse.x,mouse.y,hitCtx);
		...
		this.dispatchHitChildAll=function(children,eventType,e,cb){
		    for(var i=0,len=children.length;i<len;i++){
			    var kid=children[i];
				if(kid.dispatchEvent){
				  kid.dispatchEvent.call(kid,eventType,e);
		...
		*/
		this.eventHandler=function(canvas,eventType,e,children){
		    var mouse = this.getMouse(canvas,e);
			...
			this.hitChildAll(this.hitChildren,this.hitCtx,mouse,function(f,kid){
				...
				_this.clearCanvas(_this.hitCtx,_this.hitCan.width,_this.hitCan.height);
			...
			this.hitChildAll(children,this.hitCtx,mouse,function(f,kid){
	...
	Painter.createStage = function (arg) { return new s(arg); };
})();
