...
(function () {
    ...
    p.collectEvent = function (child) {
        if (child.listeners) {
            console.log(child.name,JSON.stringify(child.listeners))
...
(function () {
    ...
    p.collectHitedBy=function(hitArray,parent,mouse){
        var len=parent.children.length;
        parent.draw(this.hitCtx);
        var f = Painter.testHit(mouse.x, mouse.y, this.hitCtx);
        f&&hitArray.push(parent);
        if(parent.active==true){
            hitArray.push(parent);
        }
        this.clearCanvas(this.hitCtx, this.hitCan.width, this.hitCan.height);
        if(len){
            parent.children.sort(Painter.sortAscCompare('z'));
            for(var i =0;i<len;i++){
                  this.collectHitedBy(hitArray,parent.children[i],mouse);
            }
        }
    };
    p.hitChildAll = function (children,mouse, cb) {
        for (var i = children.length - 1; i > -1; i--) {
            var kid = children[i];
            kid.draw(this.hitCtx);
            var f = Painter.testHit(mouse.x, mouse.y, this.hitCtx);
            this.clearCanvas(this.hitCtx, this.hitCan.width, this.hitCan.height);
    ...
    p.eventHandler = function (canvas, eventType, e) {
        ...
        this.hitChildAll(this.hitChildren, mouse, function (f, kid) {
            if (f) {
        ...
        for (var i = 0, len = this.children.length; i < len; i++) {
            ...
            this.collectHitedBy(this.hitChildren,kid,mouse);
        }
        if(this.hitChildren.length>1){
            this.hitChildren.splice(0,this.hitChildren.length-1);

        }
        var childrenLen = this.hitChildren.length;
        if (childrenLen) {
            var hitKid = this.hitChildren[childrenLen - 1], oldHitChildrenLen = _this.oldHitChildren.length;
            if (oldHitChildrenLen) {
                for (var i = 0, len = _this.oldHitChildren.length; i < len; i++) {
                    var oldKid = _this.oldHitChildren[i];
                    if (hitKid.id != oldKid.id) {
                        //必须先执行mouseout 再执行mouseover
                        oldKid.dispatchEvent && oldKid.dispatchEvent.call(oldKid, 'mouseout', e);
                        hitKid.dispatchEvent && hitKid.dispatchEvent.call(hitKid, 'mouseover', e);
                    }
                }
            } else {
                hitKid.dispatchEvent && hitKid.dispatchEvent.call(hitKid, 'mouseover', e);
        ...
        this.initEvents = function () {
            this.defaultEvent.forEach(function (eventName, i) {
                _this.canvas.addEventListener(eventName, function (e) {
                    _this.eventHandler(_this.canvas, eventName, e);
                }, false);
            })
